Cross-compiliation of Ethanexo Core
===============================

Ethanexo Core can be cross-compiled on Linux to all other supported host systems. This is done by changing
the `HOST` parameter when building the dependencies and then specifying another `--prefix` directory when building Ethanexo.
...
When building Ethanexo Core, use
...
Note that for WSL the Ethanexo Core source path MUST be somewhere in the default mount file system, for
example /usr/src/ethanexo, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
It's not possible to build the Ethanexo Core code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the Ethanexo Core source code).
