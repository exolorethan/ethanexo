...
- [Development guidelines](#development-guidelines)
    - [General Ethanexo Core](#general-ethanexo-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Ethanexo Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Ethanexo Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/exolorethan/ethanexo/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_ethanexo
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_ethanexo --log_level=test_suite
$ valgrind -v --leak-check=full src/ethanexod -printtoconsole
```
...
# A coverage report will now be accessible at `./test_ethanexo.coverage/index.html`.
```
...
To profile a running ethanexod process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep ethanexod` -- sleep 60
```
...
Ethanexo Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Ethanexo Core, where everyone uses
...
pay attention to for reviewers of Ethanexo Core code.

General Ethanexo Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Ethanexo Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
be sent upstream, but bugfixes may also be prudent to PR against Ethanexo Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `ethanexo-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `ethanexo-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Ethanexo Core, the wallet was always
