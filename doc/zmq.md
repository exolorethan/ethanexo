...
The Ethanexo Core daemon can be configured to act as a trusted "border
router", implementing the ethanexo wire protocol and relay, making
...
The ZeroMQ feature in Ethanexo Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building ethanexod:
...
For instance:
    ...
    $ ethanexod -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/ethanexod.tx.raw \
...
These options can also be provided in ethanexo.conf.
...
From the perspective of ethanexod, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into ethanexod directly. Furthermore, no information is
...
using. ethanexod appends an up-counting sequence number to each
