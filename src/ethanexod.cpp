...
static bool AppInit(int argc, char* argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/ethanexo.conf are parsed in qt/ethanexo.cpp's main()
    SetupServerArgs();
    std::string error;
    ...
    if (HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        {
            strUsage += "\nUsage:  ethanexod [options]                     Start " PACKAGE_NAME " Daemon\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                tfm::format(std::cerr, "Error: Command line contains unexpected token '%s', see ethanexod -h for a list of options.\n", argv[i]);
                return false;
            }
        }

        // -server defaults to true for ethanexod but not for the GUI so do this here
...
#endif
            tfm::format(std::cout, "Ethanexo Core server starting\n");
...
int main(int argc, char* argv[])
{
    ...
    // Connect ethanexod signal handlers
    noui_connect();
