...
#if defined(NDEBUG)
# error "Ethanexo Core cannot be compiled without assertions."
#endif
                                     ...
                                     const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (llmq::quorumInstantSendManager->IsWaitingForTx(hash)) {
        ...
        for (const CTxIn &txin : tx.vin)
        {
            ...
            if (ptxConflicting)
            {
                // Transaction conflicts with mempool and RBF doesn't exist in Ethanexo
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    ...
    // P2SH is enforced in ETXO since block 1
...
static int64_t nTimeEthanexoSpecific = 0;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Ethanexo: Always enforce BIP30, from block 1
    assert(pindex->pprev);
    for (const auto& tx : block.vtx) {
        for (size_t o = 0; o < tx->vout.size(); o++) {
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, view, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(Ethanexo): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    // ETXO : CHECK TRANSACTIONS FOR INSTANTSEND
        ...
        for (const auto& tx : block.vtx) {
            ...
            while (llmq::CInstantSendLockPtr conflictLock = llmq::quorumInstantSendManager->GetConflictingLock(*tx)) {
                if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                    LogPrint(BCLog::ALL, "ConnectBlock(Ethanexo): chain-locked transaction %s overrides islock %s\n",
                    ...
                    return state.DoS(10, error("ConnectBlock(Ethanexo): transaction %s conflicts with transaction lock %s",
    ...
    // ETXO : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(Ethanexo): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward)) {
        return state.DoS(0, error("ConnectBlock(Ethanexo): couldn't find masternode or superblock payments"),
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeEthanexoSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - ETXO specific: %.2fms [%.2fs (%.2fms/blk)]\n", MILLI * (nTime5 - nTime4), nTimeEthanexoSpecific * MICRO, nTimeEthanexoSpecific * MILLI / nBlocksTotal);
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, int nHeight, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    FounderPayment developmentPayment = consensusParams.nDevelopmentPayment;
    CAmount developmentFundReward = developmentPayment.getFounderPaymentAmount(nHeight, blockReward);
    ...
    for (const auto& tx : block.vtx) {
    	...
    	if(nHeight > developmentStartHeight) {
		if(!developmentTransaction && developmentPayment.IsBlockPayeeValid(*tx,nHeight,blockReward)) {
			developmentTransaction = true;
		}
		} else {
			developmentTransaction = true;
		}
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    // Check proof of work RaptorHide
...
bool CChainState::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs, const CChainParams& params)
{
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, inputs, false /*fJustCheck*/, false /*fScriptChecks*/)) {
        return error("RollforwardBlock(Ethanexo): ProcessSpecialTxsInBlock for block %s failed with %s",
            pindex->GetBlockHash().ToString(), FormatStateMessage(state));
    }
    ...
    try {
        ...
        if (chainparams.NetworkIDString() == CBaseChainParams::DEVNET) {
            ...
            chainparams.DevNetGenesisBlock());
                    int nHeight = ChainActive().Height() + 1;
            bool fCheckBlock = CheckBlock(*shared_pblock, state, chainparams.GetConsensus(), nHeight);
            assert(fCheckBlock);
