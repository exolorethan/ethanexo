...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " ethanexo-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  ethanexo-tx [options] <hex-tx> [commands]  Update hex-encoded ethanexo transaction\n" +
            "or:     ethanexo-tx [options] -create [commands]   Create hex-encoded ethanexo transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded ethanexo transaction
