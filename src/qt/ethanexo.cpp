...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in ethanexo.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in ethanexo.qrc)
...
static const char* qt_argv = "ethanexo-qt";
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // ethanexo: URIs or payment requests:
        if (paymentServer) {
...
{
    QMessageBox::critical(nullptr, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. Ethanexo Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
static void SetupUIArgs()
{
    ...
    gArgs.AddArg("-windowtitle=<name>", _("Sets a window title which is appended to \"Ethanexo Core - \""), ArgsManager::ALLOW_ANY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(ethanexo);
    Q_INIT_RESOURCE(ethanexo_locale);
    ...
    /// 6. Determine availability of data and blocks directory and parse ethanexo.conf
    ...
    // ethanexo: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
