    ...
    static const std::map<std::string, RPCArg> mapParamHelp = {
        {"collateralAddress",
            {"collateralAddress", RPCArg::Type::STR, RPCArg::Optional::NO,
                "The ethanexo address to send the collateral to."}
        },
        {"collateralHash",
        ...
        {"ownerAddress",
            {"ownerAddress", RPCArg::Type::STR, RPCArg::Optional::NO,
                "The ethanexo address to use for payee updates and proposal voting.\n"
        ...
        {"payoutAddress_register",
            {"payoutAddress_register", RPCArg::Type::STR, RPCArg::Optional::NO,
                "The ethanexo address to use for masternode reward payments."}
        },
...
static void protx_register_fund_help(CWallet* const pwallet)
{
    throw std::runtime_error(
            RPCHelpMan{"protx register_fund",
                "\nCreates, funds and sends a ProTx to the network. The resulting transaction will move 1000 ETXO\n"
...
static UniValue protx_register(const JSONRPCRequest& request)
{
    ...
    if (!request.params[paramIdx + 6].isNull()) {
        ...
        if (!IsValidDestination(fundDest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid ETXO address: ") + request.params[paramIdx + 6].get_str());
    }
...
static UniValue protx_update_service(const JSONRPCRequest& request)
{
    ...
    if (!request.params[5].isNull()) {
        ...
        if (!IsValidDestination(feeSource))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid ETXO address: ") + request.params[5].get_str());
    } else {
...
static UniValue protx_revoke(const JSONRPCRequest& request)
{
    ...
    if (!request.params[4].isNull()) {
        ...
        if (!IsValidDestination(feeSourceDest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid ETXO address: ") + request.params[4].get_str());
        FundSpecialTx(pwallet, tx, ptx, feeSourceDest);
