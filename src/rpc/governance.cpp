...
static UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 ETXO irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on ethanexocentral etc. and do not save them...
...
static void gobject_vote_conf_help()
{
    throw std::runtime_error(
        RPCHelpMan{"gobject vote-conf",
            "Vote on a governance object by masternode configured in ethanexo.conf\n",
            {
...
static UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.pushKV("ethanexo.conf", statusObj);
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by masternode configured in ethanexo.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    /* ETXO features */
    { "ethanexo",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "ethanexo",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "ethanexo",               "gobject",                &gobject,                {} },
    { "ethanexo",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
