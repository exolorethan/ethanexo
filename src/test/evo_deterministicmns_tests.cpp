...
BOOST_FIXTURE_TEST_CASE(dip3_test_mempool_reorg, TestChainDIP3Setup)
{
    ...
    FundTransaction(tx_collateral, utxos, scriptCollateral, Params().GetConsensus().masternodeCollateral * COIN, coinbaseKey);
    ...
    for (size_t i = 0; i < tx_collateral.vout.size(); ++i) {
        if (tx_collateral.vout[i].nValue == Params().GetConsensus().masternodeCollateral * COIN) {
    ...
    FundTransaction(tx_reg, utxos, scriptPayout, Params().GetConsensus().masternodeCollateral * COIN, coinbaseKey);
...
BOOST_FIXTURE_TEST_CASE(dip3_test_mempool_dual_proregtx, TestChainDIP3Setup)
{
    ...
    for (size_t i = 0; i < tx_reg1.vout.size(); ++i) {
        if (tx_reg1.vout[i].nValue == Params().GetConsensus().masternodeCollateral * COIN) {
    ...
    FundTransaction(tx_reg2, utxos, scriptPayout, Params().GetConsensus().masternodeCollateral * COIN, coinbaseKey);
