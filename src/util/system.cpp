...
const char * const BITCOIN_CONF_FILENAME = "ethanexo.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "ethanexod -foo=bar
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //ethanexo-tx using stdin
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EthanexoCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EthanexoCore
    // Mac: ~/Library/Application Support/EthanexoCore
    // Unix: ~/.ethanexocore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EthanexoCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/EthanexoCore";
#else
    // Unix
    return pathRet / ".ethanexocore";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty ethanexo.conf if it does not exist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Ethanexo Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Ethanexo Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2018, nEndYear) + "The Ethanexo Core developers";
    }
    // Check for untranslated substitution to make sure Ethanexo Core copyright is not removed by accident
