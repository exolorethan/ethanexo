...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0,failfast=False, runs_ci=False):
    ...
    # Warn if ethanexod is already running (unix only)
    try:
        pidof_output = subprocess.check_output(["pidof", "ethanexod"])
        if not (pidof_output is None or pidof_output == b''):
            print("%sWARNING!%s There is already a ethanexod process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
...
class RPCCoverage():
    ...
    commands per `ethanexo-cli help` (`rpc_interface.txt`).
