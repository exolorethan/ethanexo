...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def main(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave ethanexods and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop ethanexods after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use ethanexo-cli instead of RPC for all commands")
        parser.add_argument("--ethanexod-arg", dest="ethanexod_extra_args", default=[], action="append",
                            help="Pass extra args to all ethanexod instances")
        ...
        self.config = config
        self.options.bitcoind = os.getenv("BITCOIND", default=config["environment"]["BUILDDIR"] + '/src/ethanexod' + config["environment"]["EXEEXT"])
        self.options.bitcoincli = os.getenv("BITCOINCLI", default=config["environment"]["BUILDDIR"] + '/src/ethanexo-cli' + config["environment"]["EXEEXT"])

        self.extra_args_from_options = self.options.ethanexod_extra_args
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: ethanexods were not stopped and may still be running")
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as ethanexod's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self, extra_args=None):
        ...
        if create_cache:
            ...
            # Create cache directories, run ethanexods:
            self.set_genesis_mocktime()
            for i in range(MAX_NODES):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain)  # Overwrite port/rpcport in ethanexo.conf
    ...
    def skip_if_no_bitcoind_zmq(self):
        """Skip the running test if ethanexod has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("ethanexod has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if ethanexo-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("ethanexo-wallet has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if ethanexo-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("ethanexo-cli has not been compiled.")

    def is_cli_compiled(self):
        """Checks whether ethanexo-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether ethanexo-wallet was compiled."""
...
class EthanexoTestFramework(BitcoinTestFramework):
    def set_test_params(self):
    ...
    def set_ethanexo_test_params(self, num_nodes, masterodes_count, extra_args=None, fast_dip3_enforcement=False):
        ...
        self.set_ethanexo_dip8_activation(200)

        # LLMQ default test params (no need to pass -llmqtestparams)
        self.llmq_size = 3
        self.llmq_threshold = 2

        # This is nRequestTimeout in ethanexo-q-recovery thread
    ...
    def set_ethanexo_dip8_activation(self, activate_after_block):
    ...
    def set_ethanexo_llmq_test_params(self, llmq_size, llmq_threshold):
