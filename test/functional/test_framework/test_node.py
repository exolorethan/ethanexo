...
"""Class for ethanexod node under test"""
...
class TestNode():
    """A class for representing a ethanexod node under test.
    ...
    def __init__(self, i, datadir, extra_args_from_options, *, chain, rpchost, timewait, bitcoind, bitcoin_cli, mocktime, coverage_dir, extra_conf=None, extra_args=None, use_cli=False, start_perf=False):
        ...
        self.chain = chain
        self.bitcoinconf = os.path.join(self.datadir, "ethanexo.conf")
    ...
    def __del__(self):
        # Ensure that we don't leave any ethanexod processes lying around after
        # the test ends
    ...
    def start(self, extra_args=None, *, stdout=None, stderr=None, **kwargs):
        ...
        # Add a new stdout and stderr file each time ethanexod is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by ethanexod, and
        ...
        self.running = True
        self.log.debug("ethanexod started, waiting for RPC to come up")

        if self.start_perf:
            self._start_perf()

    def wait_for_rpc_connection(self):
        """Sets up an RPC connection to the ethanexod process. Returns False if unable to connect."""
        # Poll at a rate of four times per second
        poll_per_s = 4
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                raise FailedToStartError(self._node_msg(
                    'ethanexod exited with status {} during initialization'.format(self.process.returncode)))
            try:
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcassword. ethanexod still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to ethanexod")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to ethanexod
        expected_msg: regex that stderr should match when ethanexod fails

        Will throw if ethanexod starts without an error.
        Will throw if an expected_msg is provided and it does not match ethanexod's stdout."""
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug('ethanexod failed to start: %s', e)
                self.running = False
                self.process = None
                ...
                if expected_msg is None:
                    assert_msg = "ethanexod should have exited with an error"
                else:
                    assert_msg = "ethanexod should have exited with expected error " + expected_msg
...
class TestNodeCLI():
    """Interface to ethanexo-cli for an individual node"""
    ...
    def __init__(self, binary, datadir):
        ...
        self.datadir = datadir
        self.input = None
        self.log = logging.getLogger('TestFramework.ethanexocli')

    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with ethanexo-cli command-line options
    ...
    def send_cli(self, command=None, *args, **kwargs):
        """Run ethanexo-cli command. Deserializes returned string as python object."""
        pos_args = [arg_to_cli(arg) for arg in args]
        named_args = [str(key) + "=" + arg_to_cli(value) for (key, value) in kwargs.items()]
        assert not (pos_args and named_args), "Cannot use positional arguments and named arguments in the same ethanexo-cli call"
        ...
        self.log.debug("Running ethanexo-cli command: %s" % command)
